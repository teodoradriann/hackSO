...
static int lib_prehooks(struct lib *lib)
{
	...
	if (!lib->handle) {
		sprintf(lib->outputfile, "Error: %s %s could not be executed.", lib->libname, lib->filename);
		return 1;
	}
...
static int lib_load(struct lib *lib)
{
	...
	if (strcmp(lib->funcname, "") == 0) {
		lib->funcname = strdup("run");
	}

	//printf("%s\n", lib->funcname);
	*(void **)(&lib->run) = dlsym(lib->handle, lib->funcname);
	char *error = dlerror();

	if (error != NULL) {
		sprintf(lib->outputfile, "Error: %s %s", lib->libname, lib->filename);
        dlclose(lib->handle);
        return 1;
	}
